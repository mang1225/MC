...
import lens.mdadmin.util.CheckPermissionsActivity;
import lens.mdadmin.util.SystemMethod;


public class WaterCamera2Activity extends CheckPermissionsActivity {
  ...
  private class MyPictureCallback implements PictureCallback {
    ...
    public void onPictureTaken(byte[] data, Camera camera) {
      ...
      String path = getWaterPhotoPath + photoName;
  ...
  private class MySurfaceViewCallback implements Callback {
    ...
    public void surfaceCreated(SurfaceHolder holder) {
      try {
        ...
        camera.setDisplayOrientation(getPreviewDegree(WaterCamera2Activity.this));
        camera.startPreview();
      } catch (IOException e) {
        e.printStackTrace();
      }
